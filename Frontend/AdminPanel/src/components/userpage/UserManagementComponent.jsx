...
import { FaTimes, FaUndo, FaUserLock, FaUserClock, FaUserMinus, FaSpinner } from 'react-icons/fa';
...
const UserManagementComponent = ({ 
  ...
  API_BASE_URL, 
  accessToken,
  isVisible,         // controlled by parent
  setIsVisible       // parent's setter
}) => {
  ...
  const handleDelete = () => {
    ...
    setOnConfirm(() => async () => {
      ...
      try {
        ...
        fetchUserData(); // refresh users list
      } catch (error) {
  ...
  const handleBlockOrSuspend = (actionType) => {
    ...
    // For immediate actions (unblock/unsuspend)
    if (actionType === "unblock" || actionType === "unsuspend") {
      setConfirmationMessage(`Are you sure you want to ${actionType} user ${selectedUser?.name}?`);
      setOnConfirm(() => async () => {
        setIsProcessing(true);
        try {
          await axios.put(
            `${API_BASE_URL}/admin-update-user-status`,
            { userId: selectedUser?.userId, action: actionType },
            { headers: { Authorization: `Bearer ${accessToken}` } }
          );
  ...
  const handleResetReportCount = () => {
    setConfirmationMessage(`Are you sure you want to reset report count for user ${selectedUser?.name}?`);
    setOnConfirm(() => async () => {
      setIsProcessing(true);
      try {
        await axios.put(
          `${API_BASE_URL}/reset-report-count`,
          { type: "user", id: selectedUser?.userId },
          { headers: { Authorization: `Bearer ${accessToken}` } }
        );
    ...
    <>
      ...
      <div className="fixed inset-0 overflow-hidden z-50">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onClick={handleCloseModal} />
          <div className="fixed inset-y-0 right-0 pl-10 max-w-full flex">
            <div className="w-screen max-w-md">
              <div className="h-full flex flex-col bg-white shadow-xl">
                <div className="px-4 py-6 bg-gray-50 sm:px-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-lg font-medium text-gray-900">Manage User: {selectedUser.name}</h2>
                    <button onClick={handleCloseModal} className="rounded-md text-gray-400 hover:text-gray-500 focus:outline-none">
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <div className="text-left">
                              ...
                              <div className="text-sm text-gray-500">Allow user to re-access the platform</div>
                            </div>
                          </div>
                        </button>
                      ) : (
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <div className="text-left">
                              ...
                              <div className="text-sm text-gray-500">Lift suspension from the user</div>
                            </div>
                          </div>
                        </button>
                      ) : (
      ...
      {durationModalVisible && (
        <div className="fixed inset-0 overflow-hidden z-50">
          <div className="absolute inset-0 overflow-hidden">
            <div className="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onClick={() => setDurationModalVisible(false)} />
      ...
      {confirmationModalVisible && (
        <div className="fixed inset-0 overflow-y-auto z-50">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onClick={() => setConfirmationModalVisible(false)} />
            <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Confirm Action</h3>
              <p className="text-sm text-gray-500 mb-4 whitespace-pre-line">{confirmationMessage}</p>
